...
def __traverse_node(node, opcua_client, seen):
    ...
    for n in nodes_to_monitor:
        try:
            sub = opcua_client.create_subscription(500, subhandler)
...
if __name__ == '__main__':
    ...
    opcua_client = Client("opc.tcp://10.0.1.13:48010")
    #opcua_client = Client("opc.tcp://127.0.0.1:4840")
    #opcua_client = Client("opc.tcp://10.0.1.13:48020")
    opcua_client.connect()

    device_tree_root = opcua_client.get_objects_node().get_child(["2:DeviceTree"])
    #device_tree_root = opcua_client.get_objects_node().get_child(["2:Obj1"])
