...
class MirrorWidget extends WidgetCard {
  ...
  static _getAngle (point1, point2) {
    ...
    const inner_product = (point1.x * point2.x) + (point1.y * point2.y)
    return Math.acos(inner_product /(length1 * length2))
  }
      ...
      for (let panelType in this.PANEL_NUMBERS[mirror]) {
          if (this.PANEL_NUMBERS[mirror].hasOwnProperty(panelType)) {
              ...
              for (let i = 0; i < panelNumbers.length; i++) {
                  ...
                  for (let i = 0; i < actutatorVertices.length; i++) {
                    let vertex1 = actutatorVertices[i][0]
                    let vertex2 = actutatorVertices[i][1]
  ...
  computeXYScales () {
    ...
    const min_x = d3.min(this.currentObjectData.Panel, function (d) { return d3.min(d.vertices, function (e) { return e.x }) })
    const max_x = d3.max(this.currentObjectData.Panel, function (d) { return d3.max(d.vertices, function (e) { return e.x }) })

    const min_y = d3.min(this.currentObjectData.Panel, function (d) { return d3.min(d.vertices, function (e) { return e.y }) })
    const max_y = d3.max(this.currentObjectData.Panel, function (d) { return d3.max(d.vertices, function (e) { return e.y }) })
      ...
      if (d.id === null) {
        return 'No data'
            ...
            switch (d.deviceType) {
                ...
                return 'Device State: ' + this.errorStates[d.data.State]
            }
            else if (this.viewMode === 'External Temperature') {
            switch (d.deviceType) {
                case "Panel":
    ...
    if (d.id === null) {
      return 'gray'
        ...
        switch (d.deviceType) {
          ...
          switch (d.deviceType) {
            case "MPES": {
                let misalignment = (((d.data.xCentroid - d.data.xCentroidNominal) ** 2 + (d.data.xCentroid - d.data.xCentroidNominal) ** 2) ** 0.5)
